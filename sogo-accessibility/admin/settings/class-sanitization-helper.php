...
/**
 * sogo_accessibility Sanitization Helper Class
 *
 ...
 */
class sogo_accessibility_Sanitization_Helper {
	...
	public function __construct( $sogo_accessibility ) {
		...
		add_filter( 'sogo_accessibility_settings_sanitize_text', array( $this, 'sanitize_text_field' ) );
		add_filter( 'sogo_accessibility_settings_sanitize_email', array( $this, 'sanitize_email_field' ) );
		add_filter( 'sogo_accessibility_settings_sanitize_checkbox', array( $this, 'sanitize_checkbox_field' ) );
		add_filter( 'sogo_accessibility_settings_sanitize_url', array( $this, 'sanitize_url_field' ) );
	}
	 ...
	 *
	 * Filters in order:
	 * - sogo_accessibility_settings_sanitize_<tab_slug>
	 * - sogo_accessibility_settings_sanitize_<type>
	 * - sogo_accessibility_settings_sanitize
	 * - sogo_accessibility_settings_on_change_<tab_slug>
	 * - sogo_accessibility_settings_on_change_<field_key>
	 *
	 * @since 	1.0.0
	...
	public function settings_sanitize( $input = array() ) {
		...
		add_settings_error( $this->sogo_accessibility . '-notices', $this->sogo_accessibility, __( 'Settings updated.', $this->sogo_accessibility ), 'updated' );
	...
	private function apply_type_filter( $input, $tab, $key ) {
		...
		return apply_filters( 'sogo_accessibility_settings_sanitize_' . $type, $input[$key], $key );
	}

	private function apply_general_filter( $input, $key ) {

		return apply_filters( 'sogo_accessibility_settings_sanitize', $input[$key], $key );
	}

	// Key specific on change hook
	private function do_settings_on_key_change_hook( $key, $new_value ) {

		$old_plugin_settings = get_option( 'sogo_accessibility_settings' );
		...
		if ( isset($old_plugin_settings[$key]) && $old_plugin_settings[$key] !== $new_value ) {
			...
			do_action( 'sogo_accessibility_settings_on_change_' . $key, $new_value, $old_plugin_settings[$key] );
	...
	private function do_settings_on_change_hook( $new_values, $tab ) {
		...
		if ( $changed ) {
			...
			do_action( 'sogo_accessibility_settings_on_change_' . $tab, $new_values, $old_plugin_settings );
